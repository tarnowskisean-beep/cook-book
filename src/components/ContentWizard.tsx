...
import { Tone } from '@/lib/ai/prompts';
import { generateVideo } from '@/lib/ai/mock-service';
...
export default function ContentWizard({ recipe }: Props) {
    const router = useRouter();
    const [step, setStep] = useState(1);
    const [loading, setLoading] = useState(false);
    const [saving, setSaving] = useState(false);

    // State - Now using Array for platforms
    const [platforms, setPlatforms] = useState<string[]>(['Instagram Reels']);
    // const [tone, setTone] = useState<Tone>('warm'); // Removed: Controlled by Global Persona
    ...
    const handleGenerateScript = async () => {
        ...
        try {
            // Call Server Action
            const result = await generateScript(recipe.id, platforms);

            if (result.success && result.script) {
                setScript(result.script);
                setCaption(`Check out this ${recipe.name}! 🍳 #foodie #cooking`); // Simple caption fallback for now
                setStep(2);
            } else {
    ...
    const handleSaveAndSchedule = async () => {
        setSaving(true);
        try {
            // 1. Save Content (Batch)
            const result = await saveContent(
                recipe.id,
                'VIDEO',
                videoUrl,
                platforms, // Sending array
                script
            );

            if (result.success && result.data) {
                // 2. Schedule Post (Batch)
    ...
    return (
        <div className="card" style={{ maxWidth: "800px", margin: "0 auto", minHeight: "600px" }}>
            {/* Progress Stepper */}
            ...
            {!loading && step === 1 && (
                <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-6)" }}>
                    ...
                    <div>
                        ...
                        <p style={{ marginTop: "var(--space-2)", fontSize: "0.9rem", color: "var(--text-muted)" }}>
                            Select multiple platforms to broadcast your content.
                        </p>
                    </div>
